<?php

/**
 * Check language and manage
 *
 */

namespace BCMS;

class CheckLanguageMiddleware extends \Slim\Middleware
{
    public function call()
    {
        // Get reference to application
        $app = $this->app;

        $availableLangs = array('en', 'ru');

        if ($app->availableLangs) {
            $availableLangs = $app->availableLangs;
        }

        $env = $app->environment();

        // setup default lang based on first in the list
        $lang = $availableLangs[0];

        // if they are accessing the root, you could try and direct them to the correct language
        if ($env['PATH_INFO'] == '/') {
            $langCookie = $app->getCookie('language');
            if ($langCookie and $langCookie != 'en' and in_array($langCookie, $availableLangs)) {
                $app->response->redirect('/'.$langCookie);
            }

            if (isset($env['ACCEPT_LANGUAGE'])) {
                // try and auto-detect, find the language with the lowest offset as they are in order of priority
                $priority_offset = strlen($env['ACCEPT_LANGUAGE']);

                foreach ($availableLangs as $availableLang) {
                    $i = strpos($env['ACCEPT_LANGUAGE'], $availableLang);
                    if ($i !== false && $i < $priority_offset) {
                        $priority_offset = $i;
                        $lang = $availableLang;
                    }
                }
            }
        } else {
            $pathInfo = $env['PATH_INFO'].(substr($env['PATH_INFO'], -1) !== '/' ? '/' : '');

            // extract lang from PATH_INFO
            foreach ($availableLangs as $availableLang) {
                $match = '/'.$availableLang;
                if (strpos($pathInfo, $match.'/') === 0) {
                    $lang = $availableLang;
                    $app->setCookie('language', $lang, '100 days', '/');

                    $env['PATH_INFO'] = substr($env['PATH_INFO'], strlen($match));
                    if ($lang == 'en') {
                        $app->response->redirect($env['PATH_INFO']);
                    }

                    if (strlen($env['PATH_INFO']) == 0) {
                        $env['PATH_INFO'] = '/';
                    }

                }
            }
        }

        //echo '<!-- '.$lang. '-->';

        $app->locale = $lang;

        //$app->setCookie('language', $app->locale, '100 days', '/');
        //$app->response->setCookie('language2', $lang, '100 days', '/');

        $app->view->appendData(array('lang' => $lang));
        // Run inner middleware and application
        $this->next->call();

    }
}
